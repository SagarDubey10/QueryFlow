...
def convert_nl_to_sql(nl_text):
    if not client: return "ERROR: Gemini API Key is missing. Please check your .env file."
...
Rules:
1. Return ONLY the raw SQL query. No explanations.
2. Write the SIMPLEST possible SQL to answer the query.
3. ALWAYS format the SQL cleanly with line breaks for readability.
4. If unrelated to the database, return: ERROR: Unrelated query.
...
User query: {nl_text}"""
    ...
    try:
        ...
        print(f"\n--- API ERROR ---\n{str(e)}\n-----------------\n")
        return "ERROR: Our AI assistant is experiencing connection issues. Please try again."
...
def query():
    ...
    if not sql_query.startswith("ERROR"):
        if not is_safe_query(sql_query):
            error_msg = "🛡️ Security Alert: QueryFlow is currently in Read-Only mode."
        else:
            try:
                conn = sqlite3.connect('database.db')
...
def export_csv():
    ...
    if not sql_query or not is_safe_query(sql_query): return "Invalid query", 400
    try:
        conn = sqlite3.connect('database.db')
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
        ...
        output = si.getvalue()
        return Response(output, mimetype="text/csv", headers={"Content-Disposition": "attachment;filename=queryflow_export.csv"})
    except Exception as e:
